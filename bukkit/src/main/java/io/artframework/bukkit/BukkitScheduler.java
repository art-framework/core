...
import static io.artframework.bukkit.util.TimeUtil.millisToTicks;
...
public class BukkitScheduler implements Scheduler {
    ...
    public int scheduleSyncDelayedTask(Runnable task, long delay) {
        return scheduler.scheduleSyncDelayedTask(plugin, task, millisToTicks(delay));
    }
    ...
    public int scheduleSyncRepeatingTask(Runnable task, long delay, long period) {
        return scheduler.scheduleSyncRepeatingTask(plugin, task, millisToTicks(delay), millisToTicks(period));
    }
    ...
    public Task runTaskLater(Runnable task, long delay) throws IllegalArgumentException {
        return BukkitTask.of(scheduler.runTaskLater(plugin, task, millisToTicks(delay)));
    }

    public Task runTaskLaterAsynchronously(Runnable task, long delay) throws IllegalArgumentException {
        return BukkitTask.of(scheduler.runTaskLaterAsynchronously(plugin, task, millisToTicks(delay)));
    }

    public Task runTaskTimer(Runnable task, long delay, long period) throws IllegalArgumentException {
        return BukkitTask.of(scheduler.runTaskTimer(plugin, task, millisToTicks(delay), millisToTicks(period)));
    }

    public Task runTaskTimerAsynchronously(Runnable task, long delay, long period) throws IllegalArgumentException {
        return BukkitTask.of(scheduler.runTaskTimerAsynchronously(plugin, task, millisToTicks(delay), millisToTicks(period)));
    }
}