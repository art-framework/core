...
import net.silthus.art.ParserProvider;
...
public class DefaultParserProvider extends AbstractProvider implements ParserProvider {

    private final Map<Class<?>, Supplier<Parser<?>>> parsers = new HashMap<>();

    public DefaultParserProvider(@NonNull Configuration configuration) {
        super(configuration);
    }
    ...
    public <TInput> ParserProvider add(Class<TInput> inputType, Supplier<Parser<TInput>> parserSupplier) {
        parsers.put(inputType, (Supplier) parserSupplier);
        return this;
    }

    @Override
    public ParserProvider remove(Class<?> inputType) {
