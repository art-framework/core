...
public abstract class ArtContext<TTarget, TConfig, TContextOptions extends ArtObjectConfig<TConfig>> {
    ...
    public ArtContext(Storage storage, Class<TTarget> targetClass, TContextOptions config) {
    ...
    /**
     * Stores a persistent value for the given target in the store using the {@link Storage}.
     ...
     * @see Storage#store(ArtContext, Target, String, Object)
     */
    public <TValue> void store(Target<?> target, String key, TValue value) {
        key = StorageConstants.LOCAL_KEY_PREFIX + key;
        getStorage().data(this, target, key, value);
    }

    /**
     * Retrieves data stored with {@link #store(Target, String, Object)} from the {@link Storage}.
     *
    ...
    public <TValue> Optional<TValue> get(Target<TTarget> target, String key, Class<TValue> valueClass) {
        key = StorageConstants.LOCAL_KEY_PREFIX + key;
        return getStorage().get(this, target, key, valueClass);
    }
}
